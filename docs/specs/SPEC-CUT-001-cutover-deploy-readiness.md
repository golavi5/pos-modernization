...
**Status**: APPROVED — 2026-08-10. ⚠️ **CORRECCIÓN 2026-08-11: la afirmación "app-level readiness is complete" de abajo quedó desmentida por el dry-run sobre Coolify.** Ese run encontró dos defectos **de nivel de aplicación** que impiden registrar una venta — `GET /products` devuelve 400 siempre (`@Transform` a booleano + `@IsString()` + valor por defecto en `product-query.dto.ts`), y las 7 rutas de `lib/api/sales.ts` apuntan a `/sales*` cuando el backend sirve `/sales/orders*`. Ninguno es de infraestructura, así que pertenecen a esta spec, no a CUT-002; el pre-flight local de 2026-06-30 que sustenta el "GO app-level" no los detectó porque nunca ejercitó el catálogo ni la venta contra la API real. El token se queda en APPROVED porque el guard monotónico de Kairos no puede retroceder — **léase como "construido, con dos bloqueantes abiertos", no como listo**. Detalle y evidencia: [`STAGING-DRY-RUN-RESULTS.md`](../../new-implementation/STAGING-DRY-RUN-RESULTS.md#dry-run-4-5-sobre-coolify--2026-08-11--veredicto--no-go). Lo que sigue es el texto previo, conservado como historia. **The infrastructure gates are re-homed to `SPEC-CUT-002`, which exists for exactly that scope.** §3 blockers B-05..B-10 closed and exercised end-to-end on a local prod-parity stack (`NODE_ENV=production`, `DB_RUN_MIGRATIONS=true`, fail-fast secret validation, no demo seed) — record and app-level **GO** verdict in `../../new-implementation/STAGING-DRY-RUN-RESULTS.md` (2026-06-30). §6 exit criteria **1/5 closed (bullet 2), bullet 4 partial**: bullet 2 — S-01's only deferred item was the lint gate, which `SPEC-BACK-002` shipped 2026-08-10 (PR #51/#52) and which is now a **required** check (`gh api …/branches/main/protection` → contexts include "Lint — backend + frontend"; observed gating PR #53); S-05 was already ✅. Bullet 4 is **half done**: backup/restore is demonstrated (S-03: real dump→restore round-trip against MySQL 8, row counts match), rollback-*by-redeploy* is a Coolify mechanic and stays open in CUT-002 — do not read bullet 4 as closed. §4 re-audited against code 2026-08-09: S-03..S-08 and S-10 done (S-10 was already fixed when checked), S-02 (Sentry) deferred by decision, S-09 re-homed to SPEC-FRONT-001/002. Backend suite 246/246 green at that audit. **Not DONE, and not walkable back:** bullets 1, 3 and 5 say "verified in staging" / "pass on staging", and calling those satisfied by a local run would redefine the criteria after the fact. The four gates a local stack provably cannot cover (real `app.`/`api.` subdomain CORS, MySQL port-not-exposed, rollback-by-redeploy, Coolify healthcheck/observability) are enumerated in `SPEC-CUT-002` §2; promote this to DONE when that spec records its Go/No-Go. Header was stale DRAFT.
**Owner**: gandhi
**Created**: 2026-06-16
...
|----|------|-----------------|
| S-01 | ✅ **DONE — CI gating** (lint gate closed 2026-08-10 by `SPEC-BACK-002`, PR #51/#52) | Root cause: the only workflow sat at `new-implementation/.github/` (a subdir), so GitHub **never ran it** (`gh run list` empty). Moved to repo-root `.github/workflows/ci.yml` with gate jobs **backend (npm ci → test → build)** and **frontend (npm ci → build = typecheck)** on **Node 20** (matches Dockerfiles), v4 actions. Verified locally: backend `npm test` 173/173 + `build` green; frontend `build` green. E2E kept but `continue-on-error` (port wiring fixed: backend :3001, frontend :3000; full prod env + bootstrap) — not gating until validated on a runner. ~~**Deferred:** lint gate — neither app ships an ESLint config (`next lint`/`eslint` have nothing to run); add configs first.~~ **Closed 2026-08-10:** `SPEC-BACK-002` shipped ESLint configs for both apps plus a non-mutating `lint:ci`, and `Lint — backend + frontend` is now a **required** status check — `gh api …/branches/main/protection --jq '.required_status_checks.contexts'` → `["Backend — test + build","Frontend — build","Frontend — i18n checks","Lint — backend + frontend"]`. Observed gating a real PR (#53). |
...
| S-03 | ✅ **DONE — backup / restore / rollback** | `scripts/db-backup.sh` (mysqldump `--single-transaction` → gzip, timestamped, retention prune; `MYSQL_PWD` so no password on the cmdline) + `scripts/db-restore.sh` (destructive, confirm-guarded). `DEPLOYMENT-COOLIFY.md` documents Coolify-native backups as primary, the scripts + a cron example as fallback, restore-into-scratch-first, and a **rollback** procedure that flags migrations are **forward-only** (restore a pre-migration backup or `migration:revert`). **Verified:** real dump→restore round-trip against MySQL 8 — row counts match. |
...
| S-05 | ✅ **Done (untracked) — history scrub deliberately skipped** | Two env files were committed historically: `new-implementation/.env` (`MYSQL_*` only) and **`new-implementation/backend/.env`** (added in `78e036c4`, untracked in `eb7f5afc`) which held **real-looking secrets** — 64-char `JWT_SECRET` + `JWT_REFRESH_SECRET` and a `DB_PASSWORD`. Both are now untracked + gitignored; `.env.example` templates + `cp` docs in place; future leakage stopped. **History scrub NOT done, by decision:** (1) the repo was **never deployed**, so those secrets were never operational — nothing to rotate, nothing compromised; (2) a local rewrite + force-push would **not** purge GitHub anyway (commit `78e036c4` stays reachable by SHA + merged-PR refs — only GitHub Support can purge); (3) it's destructive for ~zero benefit on dead secrets. **The real control is at deploy:** generate fresh secrets (`openssl rand -base64 48`). `main.ts` `validateProductionEnv()` + the `CHANGE_ME` placeholders make an *un-edited* template fail fast — but they do **not** block deliberately copying the leaked real values (those pass validation), so "generate fresh, never reuse the historical values" is a **procedural** rule in the deploy docs, not an enforced one. Optional future hygiene: local rewrite of both `.env` files + GitHub Support purge. |
