...
**Status**: DONE — 2026-08-11. Sus §6 exit criteria quedan cubiertos por el dry-run completo que cerró `SPEC-CUT-002` con **GO firmado** ([acta](../../new-implementation/STAGING-DRY-RUN-RESULTS.md)): B-05..B-10 ejercitados en la instancia real, S-01 y S-05 cerrados, smoke y aislamiento verificados, backup+restore+rollback demostrados sobre la instancia, y los S-items restantes triados como fast-follow en el acta. **La corrección del 2026-08-11 sigue vigente en su parte de fondo:** el pre-flight local de junio no detectó los defectos de aplicación que sí encontró el dry-run sobre Coolify — un pre-flight que no ejercita el catálogo ni la venta contra la API real no sustituye a un despliegue. Esos defectos (D1, D2, D5, D6, D7, D8) se cerraron en `SPEC-BACK-003`. Queda abierto **D3** como fast-follow (`SPEC-BACK-004`). Lo que sigue es el texto previo, conservado como historia.
**Owner**: gandhi
**Created**: 2026-06-16
