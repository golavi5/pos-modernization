...
| S-09 | ⚠️ **Open — larger than written here** | *Re-audited 2026-08-09; the original text was wrong on both count and files.* It named product detail/edit + category pages, but `products/[id]/page.tsx` and `products/[id]/edit/page.tsx` **already use `useTranslations`**. The untranslated product pages are the **list**, **categories** and **new** pages, and they carry **mixed EN and ES** literals ("Create Product" beside "Buscar productos..."). Real scope: **9 of 76** component/page files use `useTranslations`; ~24 carry hardcoded strings. Now owned by **`SPEC-FRONT-001`** (app shell + those three pages) with the remaining ~20 files split out as **SPEC-FRONT-002**. Not a go-live blocker. |
